...
import { gitDiff, type DiffMode } from "../workspace/git.js";
...
export interface PlanPacketOptions extends CommonPacketOptions {}

export interface ReviewPacketOptions extends CommonPacketOptions {
  tests?: string;
  notes?: string;
  diffMode?: DiffMode;
}
...
export function parseFileSelection(value: string): FileSelection {
  const hash = value.lastIndexOf("#");
  if (hash < 0) return { path: value };
  const path = value.slice(0, hash);
  const range = value.slice(hash + 1);
  const match = range.match(/^(\d+)(?:-(\d+))?$/);
  if (!path || !match) return { path: value };
  const startLine = Number(match[1]);
  const endLine = match[2] ? Number(match[2]) : startLine;
  return { path, startLine, endLine };
}

async function collectFileSections(
  workspace: Workspace,
  selections: FileSelection[],
  charBudget: number
): Promise<{ text: string; included: string[]; omitted: string[]; redactions: number; truncated: boolean }> {
  ...
  for (const selection of selections) {
    try {
      ...
      included.push(label);
      used += section.length;
      if (file.truncated && selection.endLine === undefined) truncated = true;
    } catch (error) {
...
function header(state: "INIT" | "EXECUTED", options: CommonPacketOptions): string {
...
export async function buildPlanPacket(workspace: Workspace, options: PlanPacketOptions): Promise<PacketResult> {
  ...
  if (packet.length > maxChars) packet = packet.slice(0, maxChars - 24) + "\n[PACKET TRUNCATED]\n";
  return {
    packet,
...
export async function buildReviewPacket(workspace: Workspace, options: ReviewPacketOptions): Promise<PacketResult> {
  const maxChars = boundedMaxChars(options.maxChars);
